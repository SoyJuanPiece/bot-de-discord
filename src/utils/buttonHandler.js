...
async function handleShopButton(interaction, client, buttonType) {
    if (buttonType === 'refresh') {
        ...
        if (!interaction.deferred && !interaction.replied) {
            await interaction.update({ embeds: [shopEmbed], components: interaction.message.components });
        }
...
async function handleTicketButton(interaction, client, params) {
    ...
    if (ticketAction === 'close') {
        ...
        if (!ticket) {
            return interaction.reply({ content: '❌ Este ticket no existe en la base de datos.', ephemeral: true });
        }
        ...
        await interaction.reply({
            content: '➕ Usa `/ticket asignar [@usuario]` para agregar alguien al ticket.',
            ephemeral: true
        });
    }
}
